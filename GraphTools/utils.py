...
def normalized_rmse(y_true, y_pred, norm_type='range'):
    ...
    if norm_type == 'range':
        ...
        # Normalize by the max absolute value of y_true. Used for graphon 11
        # (SBM) where 'range' can be misleading because the analytic centrality
        # is a step function with very few distinct levels.
        norm_factor = np.max(np.abs(y_true))
        if norm_factor == 0:
            raise ValueError("Normalization by max is not possible because max|y_true|=0.")
    else:
        raise ValueError("Unknown norm_type. Use 'range', 'mean', 'std', or 'max'.")
